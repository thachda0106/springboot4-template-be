 ...
 * expired token -> 401, wrong issuer/audience -> 401, garbage token -> 401.
 */
class JwtValidationIntegrationTest extends AbstractApiIntegrationTest {
    ...
    void tokenSignedWithWrongKeyIsRejected() throws Exception {
        ...
        String token = mint(userId, "USER", wrongSecret, Instant.now().plusSeconds(3600));
    ...
    void expiredTokenIsRejected() throws Exception {
        ...
        String token = mint(userId, "USER", jwtSecret, Instant.now().minusSeconds(600));
    ...
    void tokenWithWrongIssuerIsRejected() throws Exception {
        String userId = createUser("jwt-dave");

        String token = mint(userId, "USER", jwtSecret, Instant.now().plusSeconds(3600), "wrong-issuer",
                "modular-monolith");
    ...
    void tokenWithWrongAudienceIsRejected() throws Exception {
        String userId = createUser("jwt-erin");

        String token = mint(userId, "USER", jwtSecret, Instant.now().plusSeconds(3600), "modular-monolith",
                "some-other-audience");
    ...
    private String mint(String subject, String role, String secret, Instant expiresAt) throws Exception {
        return mint(subject, role, secret, expiresAt, "modular-monolith", "modular-monolith");
    }

    private String mint(String subject, String role, String secret, Instant expiresAt,
