...
/**
 * Unit tests for the burst layer ({@link ThrottleFilter}): per-IP behavior,
 ...
 * context, no Docker.
 */
class ThrottleFilterTest {

    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    private ThrottleFilter filter(int limit, Duration refreshPeriod, Long... counts) {
        ...
        RedisFixedWindowRateLimiter limiter = new RedisFixedWindowRateLimiter("throttle", limit,
                refreshPeriod, true, template, new SimpleMeterRegistry());
        return new ThrottleFilter(limiter, refreshPeriod, OBJECT_MAPPER);
    }
    ...
    @Test
    void passesRequestsUpToTheBurstLimit() throws Exception {
        ThrottleFilter filter = filter(3, Duration.ofSeconds(1), 1L, 2L, 3L);
    ...
    @Test
    void rejectsTheRequestPastTheBurstLimitWithThrottledError() throws Exception {
        ThrottleFilter filter = filter(2, Duration.ofSeconds(1), 1L, 2L, 3L);
        ...
        assertThat(code).isEqualTo("THROTTLED");
    }

    @Test
    void ipsAreLimitedIndependently() throws Exception {
        ThrottleFilter filter = filter(1, Duration.ofSeconds(1), 1L, 2L, 1L);
    ...
    void actuatorPathsPassThroughUnlimited() throws Exception {
        ThrottleFilter filter = filter(1, Duration.ofSeconds(1), 1L, 2L);

        // Burn the budget for this IP, then check the actuator path still passes.
